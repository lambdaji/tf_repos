...
def preprocess(datadir, outdir):
    ...
    # for validation.
    with open(FLAGS.output_dir + 'tr.csv', 'w') as out_train:
        with open(FLAGS.output_dir + 'va.csv', 'w') as out_valid:
            with open(FLAGS.input_dir + 'train.txt', 'r') as f:
                for line in f:
                    ...
                    for i in range(0, len(continous_features)):
                        ...
                        feat_vals.append("{0:.6f}".format(val).rstrip('0').rstrip('.'))
    ...
    with open(FLAGS.output_dir + 'te.csv', 'w') as out:
